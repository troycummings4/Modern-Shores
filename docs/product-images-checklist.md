...
48 products, each needs one real image URL from its Amazon listing. Use
...
2. Click a product link below. A **SiteStripe toolbar** appears at the top
   of the page (only shows up when you're logged in as an approved
   Associate).
3. Click **Image** in that toolbar (sometimes under "Get Link" → "Image").
4. Copy the image URL it gives you (usually starts with
   `https://m.media-amazon.com/images/...` or similar).
...
send the edited file) and it gets wired into the site in one pass. You
don't have to do all 48 at once — partial lists work too; anything left
...
- [ ] `full-suspension-electric-mountain-bike` — Full-Suspension Electric Mountain Bike
